...
router.get('/domains', auth.findClient, async (req: auth.CheckedRequest, res) => {
  const domains = await req.client.getDomains();
  res.send(domains);
});

router.get('/domains/:domainId/talks', auth.findClient, async (req: auth.CheckedRequest, res) => {
...
router.post('/domains/:domainId/talks/:talkId/messages', auth.findClient, async (req: auth.CheckedRequest, res) => {
  const q = req.params;
